...
#  echo "   -chezmoi        ???"
  #echo "   -circleci       Use CircleCI SaaS"
   ...
   echo "# USAGE EXAMPLES:"
   echo "chmod +x mac-setup.sh   # change permissions"
   echo "./mac-setup.sh          # display this menu"
   echo "./mac-setup.sh -usage   # display usage examples"
}  # args_prompt()

### 03. Usage Examples
usage_examples() {
   echo "./mac-setup.sh -usage  # USAGE EXAMPLES:"
   echo "./mac-setup.sh -init -v # to download .env & aliases into \$HOME folder ${HOME}"
   echo "./mac-setup.sh -init -gfb \"~/github-wilsonmar\" -ssh -v  # create GITHUB_FOLDER_BASE"
   echo "./mac-setup.sh -mount \"V-64G-4\" -UM -v  # mount USB drive and backup to it"
   echo " "
   echo "./mac-setup.sh -I -U -utils -v  # install or update utilities spec'd in .env file"
   echo "# Using default configuration settings downloaed to \$HOME/mac-setup.env "
   echo " "
   echo "./mac-setup.sh -python -I -U -file \"~/github-wilsonmar\" -v "
   echo "./mac-setup.sh -golang -I -U -v "
   echo "./mac-setup.sh -v -Consul -k -a -K   # Use HashicorpVault in Docker for localhost Kept alive"
   echo "./mac-setup.sh -v -Consul -podman -a -K   # Use HashicorpVault in Podman for localhost Kept alive"
   echo "./mac-setup.sh -v -k -HV -a -K   # Use HashicorpVault in Docker for localhost Kept alive"
   echo "./mac-setup.sh -v -HV -m -ts     # Use HashicorpVault -testserver"
   echo "./mac-setup.sh -v -HV -s -ts     # Initiate Vault testserver"
   echo " "
   echo "./mac-setup.sh -v -aws  # for Terraform"
   echo "./mac-setup.sh -v -eks -DA "
   echo "./mac-setup.sh -v -g \"abcdef...89\" -p \"cp100-1094\"  # Google API call"
   echo " "
   echo "./mac-setup.sh -v -I -U -c    -HV -G -N \"python-samples\" -f \"a9y-sample.py\" -P \"-v\" -t -AWS -C  # Python sample app using Vault"
   echo "./mac-setup.sh -v -venv -c -T -F \"section_2\" -f \"2-1.ipynb\" -K  # Jupyter Conda Tensorflow in Venv"
   echo "./mac-setup.sh -v -DA -M -C"
   echo "./mac-setup.sh -G -v -f \"challenge.py\" -P \"-v\"  # to run a program in my python-samples repo"
   ...
   echo "./mac-setup.sh -pike -c -gru \"alfonsof/terraform-azure-examples\" -d1 -gfn \"alonzo\" -F \"code/01-hello-world\" -o -v "
   echo "./mac-setup.sh -tf -F \"tf-module1\" -d1 -c -v -I -U"
   echo "./mac-setup.sh -steampipe -v -I -aws -c -d1 -gfn \"steampipe\" -of "
   echo "./mac-setup.sh -vsc -v -a -U"
   echo "./mac-setup.sh -akeyless -v -a -U"
   echo "./mac-setup.sh -gatewayd \"filename.gz\"-pfn \"work\" -DB -v"
   echo "./mac-setup.sh -down -url \"https://.../.../filename.gz\" -v"
   echo "./mac-setup.sh -sha -pfn \"work\" -f \"filename.gz\" -unzip  -hash \"...\" -of \"gatewayd\" -v"
   echo "./mac-setup.sh -kraft \"fra0\" -v"
} # usage_examples()
...
# ./mac-setup.sh -pfn "work" -url "https://github.com/gatewayd-io/gatewayd/releases/download/v0.8.11/gatewayd-darwin-amd64-v0.8.11.tar.gz" -v
...
if [ "${USE_AIAC}" = true ]; then  # -tf
   ...
   if [ -z "${OPENAI_API_KEY}" ]; then  # not defined in ~/mac-setup.sh
...
# ./mac-setup.sh -sha -pfn "work" -f "gatewayd-darwin-amd64-v0.8.11.tar.gz" -unzip -of "gatewayd" -DB -v
